...
public class MyRatingPlugin implements RatePlugin {
    ...
    private double getRatingFactor(PersonalVehicle vehicle, PersonalAutoQuote quote, String coverageType) {
        double stateFactor = switch (vehicle.vehicleLicenseState()) {
        ...
        double damageFactor = switch (vehicle.vehicleDamage()) {
        ...
        double licenceFactor = switch (vehicle.primaryDriverLicence()) {
        ...
        double excessFactor = switch (vehicle.excess()) {
        ...
        double usageFactor = switch (vehicle.vehicleUsage()) {
        ...
        int vehicleYear = vehicle.redbook() != null ? vehicle.redbook().year() : 2020;
        ...
        LocalDate dob = vehicle.driver() != null ? vehicle.driver().dateOfBirth() : null;
        ...
        int claims = 0;
        try {
            String claimStr = quote.data().priorClaims(); 
    ...
    private RatingItem rateFire(PersonalVehicle vehicle) {
        double baseRate = 100.0;
        double finalRate = baseRate * getRatingFactor(vehicle, "Fire");
        return RatingItem.builder()
    ...
    private RatingItem rateTheft(PersonalVehicle vehicle) {
        double baseRate = 100.0;
        double finalRate = baseRate * getRatingFactor(vehicle, "Theft");
        return RatingItem.builder()
    ...
    private RatingItem rateOwnDamage(PersonalVehicle vehicle) {
        double baseRate = 200.0;
        double finalRate = baseRate * getRatingFactor(vehicle, "OwnDamage");
        return RatingItem.builder()
    ...
    private RatingItem rateThirdParty(PersonalVehicle vehicle) {
        double baseRate = 80.0;
        double finalRate = baseRate * getRatingFactor(vehicle, "ThirdParty");
        return RatingItem.builder()
    ...
    public RatingSet rate(PersonalAutoQuoteRequest request) {
        ...
        for(PersonalVehicle vehicle : personalAutoQuote.personalVehicles()) {
            ...
            if (vehicle.ownDamage() != null) {
                ratingItems.add(ratingItemsrateOwnDamage(vehicle));
            }
            if (vehicle.fire() != null) {
                ratingItems.add(ratingItemsrateFire(vehicle));
            }
            if (vehicle.theft() != null) {
                ratingItems.add(ratingItemsrateTheft(vehicle));
            }
            if (vehicle.thirdParty() != null) {
                ratingItems.add(ratingItemsrateThirdParty(vehicle));
            }
            if (vehicle.windscreen() != null) {
                ratingItems.add(ratingItemsrateWindscreen(vehicle));
            }
            if (vehicle.babySeat() != null) {
                ratingItems.add(ratingItemsrateBabySeat(vehicle));
            }
        }
        
        if (ratingItems.size() > 0) {
            ratingItems.add(flatRateWithTaxes(request.transport().locator(), 35));
        }
    ...
    public RatingSet rate(PersonalAutoRequest request) {
        ...
        if (ratingItems.size() > 0) {
            request.segment().ifPresent(s-> ratingItems.add(flatRateWithTaxes(segment.transport().locator(), 35)));
        }
